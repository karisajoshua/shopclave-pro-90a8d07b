...
const VendorProducts = () => {
  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        title="Products"
        subtitle="Manage your live catalogue, drafts, and stock levels."
      ...
      {!products?.length ? (
        <div className="admin-card">
          <AdminEmptyState
            ...
            onAction={() => (window.location.href = "/vendor/products/new")}
          />
        </div>
      ) : (
        <>
    ...
    <tr>
      <td>
        <div className="flex items-center gap-2.5">
          <img src={p.product_images?.[0]?.url || "/placeholder.svg"} alt="" className="w-9 h-9 rounded-md object-cover bg-secondary border border-border shrink-0" />
      ...
      <td>
        {editStock ? (
          ...
          <button onClick={() => setEditStock(true)} className={`hover:underline ${p.stock < 5 ? "text-destructive font-medium" : ""}`}>{p.stock}</button>
        )}
      </td>
      <td>
        <span className={`status-pill ${statusColor(p.status)}`}>{p.status}</span>
